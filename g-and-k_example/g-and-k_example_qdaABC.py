...
def simulate_for_observed(obs_data, obs_id, n_simulations):
    ...
    def simulate_once(seed):
        ...
        if dist_type == 0:
            ...
            sim_data = np.random.normal(0, 1 + 0.5 * rng.random(), size=len(obs_data))
            param = np.std(sim_data)
...
def summarize_results(folder_path):
    ...
    np.savetxt(os.path.join(folder_path, 'gk_example_g0_QDA_probabilities.csv'), np.array(qda_frequencies), delimiter=',', header='QDA', comments='')
    np.savetxt(os.path.join(folder_path, 'gk_example_g0_QDA_params.csv'), np.array(qda_means), delimiter=',', header='QDA', comments='')
