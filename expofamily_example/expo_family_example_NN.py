...
def simulate_datasets(num_datasets=10**6, sample_size=100):
    n_per_model = num_datasets // 3
    expo_samples = np.random.exponential(scale=1.0, size=(n_per_model, sample_size))
    logn_samples = np.random.lognormal(mean=0, sigma=1.0, size=(n_per_model, sample_size))
    ...
    y_params = np.vstack([np.mean(expo_samples, axis=1).reshape(-1, 1),
                          np.mean(logn_samples, axis=1).reshape(-1, 1),
                          np.mean(gamma_samples, axis=1).reshape(-1, 1)])
...
def train_model(model, X, y_class, y_params, output_dir, epochs=10, batch_size=32):
    ...
    model.save(os.path.join(output_dir, 'expo_family_model.keras'))
    print(f"Model saved in {output_dir}/expo_family_model.keras")
    return model
...
def predict_observed(model_path, observed_path, output_dir):
    ...
    pd.DataFrame(pred_params, columns=['NN_Param_Estimate']).to_csv(os.path.join(output_dir, 'expo_family_exp_NN_params.csv'), index=False)
...
def main():
    ...
    if args.predict:
        model_path = os.path.join(model_output, 'expo_family_model.keras')
