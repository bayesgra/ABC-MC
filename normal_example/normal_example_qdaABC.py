...
def simulate_for_observed(obs_data, obs_id, n_simulations):
    ...
    def simulate_once(seed):
        ...
        if dist_type == 0:
            sim_data = np.random.normal(0, 1, size=len(obs_data))
            param = 0.0
        else:
            mu = rng.normal(0, 10)
            sim_data = np.random.normal(mu, 1, size=len(obs_data))
            param = mu
...
def main():
    ...
    if not os.path.exists(observed_path):
        raise FileNotFoundError(f"Missing observed datasets at {observed_path}. Run normal_example_distanceABC.py first.")
...
def summarize_results(folder_path):
    ...
    np.savetxt(os.path.join(folder_path, 'normal_model0_QDA_probabilities.csv'), np.array(qda_frequencies), delimiter=',', header='QDA', comments='')
    np.savetxt(os.path.join(folder_path, 'normal_model0_QDA_mean_mu.csv'), np.array(qda_means), delimiter=',', header='QDA', comments='')
